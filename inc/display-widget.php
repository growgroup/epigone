...
class DWPlugin
{
	...
	function hidden_widget_options($widget, $return, $instance)
	{
		...
		?>
		<div class="dw_opts">
			<input type="hidden" name="<?php echo $widget->get_field_name('dw_include'); ?>"
				   id="<?php echo $widget->get_field_id('dw_include'); ?>"
				   value="<?php echo $instance['dw_include'] ?>"/>
			<input type="hidden" id="<?php echo $widget->get_field_id('dw_logged'); ?>"
				   name="<?php echo $widget->get_field_name('dw_logged'); ?>"
				   value="<?php echo $instance['dw_logged'] ?>"/>
			...
			<?php foreach ($instance as $k => $v) {
				...
				if (strpos($k, 'page-') === 0 || strpos($k, 'type-') === 0 || strpos($k, 'cat-') === 0 ||
					...
					?>
					<input type="hidden" id="<?php echo $widget->get_field_id($k); ?>"
						   name="<?php echo $widget->get_field_name($k); ?>" value="<?php echo $v ?>"/>
				<?php } ?>
			<?php } ?>

			<input type="hidden" name="<?php echo $widget->get_field_name('other_ids'); ?>"
				   id="<?php echo $widget->get_field_id('other_ids'); ?>"
				   value="<?php echo esc_attr($instance['other_ids']) ?>"/>
			<input type="hidden" name="<?php echo $widget->get_field_name('mobile_detect_disp'); ?>"
				   id="<?php echo $widget->get_field_id('mobile_detect_disp'); ?>"
				   value="<?php echo esc_attr($instance['mobile_detect_disp']) ?>"/>
		</div>
		<?php
	}
	...
	function show_hide_widget_options($widget, $return, $instance)
	{
		...
		<div class="contents-colspan">
			...
			<p>
				<label
					for="<?php echo $widget->get_field_id('dw_include'); ?>"><?php _e('表示するユーザー', 'display-widgets') ?></label>
				<select name="<?php echo $widget->get_field_name('dw_logged'); ?>"
						id="<?php echo $widget->get_field_id('dw_logged'); ?>" class="widefat">
					<option value=""><?php _e('すべてのユーザー', 'display-widgets') ?></option>
					<option
						value="out" <?php echo selected($instance['dw_logged'], 'out') ?>><?php _e('ログアウトユーザー', 'display-widgets') ?></option>
					<option
						value="in" <?php echo selected($instance['dw_logged'], 'in') ?>><?php _e('ログインユーザー', 'display-widgets') ?></option>
				</select>
			</p>
			...
			<p><label
					for="<?php echo $widget->get_field_id('dw_include'); ?>"><?php _e('表示条件', 'display-widgets') ?></label>
			</p>
			<select name="<?php echo $widget->get_field_name('dw_include'); ?>"
					id="<?php echo $widget->get_field_id('dw_include'); ?>" class="widefat">
				<option value="0"><?php _e('チェックしたページで表示しない', 'display-widgets') ?></option>
				<option
					value="1" <?php echo selected($instance['dw_include'], 1) ?>><?php _e('チェックしたページで表示する', 'display-widgets') ?></option>
			</select>
			</p>

			<div style="height:150px; overflow:auto; border:1px solid #dfdfdf; padding:5px; margin-bottom:5px;">
				<h4 class="dw_toggle" style="cursor:pointer;margin-top:0;"><?php _e('主要ページ', 'display-widgets') ?>
					+/-</h4>
				...
				<div class="dw_collapse">
					<?php foreach ($wp_page_types as $key => $label) {
						...
						?>
						<p><input class="checkbox" type="checkbox" <?php checked($instance['page-' . $key], true) ?>
								  id="<?php echo $widget->get_field_id('page-' . $key); ?>"
								  name="<?php echo $widget->get_field_name('page-' . $key); ?>"/>
							<label
								for="<?php echo $widget->get_field_id('page-' . $key); ?>"><?php echo $label . ' ' . __('Page', 'display-widgets') ?></label>
						</p>
					<?php } ?>
				</div>
				...
				<?php if (!empty($this->cposts)) { ?>
					...
					<div class="dw_collapse">
						<?php foreach ($this->cposts as $post_key => $custom_post) {
							...
							<p><input class="checkbox"
									  type="checkbox" <?php checked($instance['type-' . $post_key], true) ?>
									  id="<?php echo $widget->get_field_id('type-' . $post_key); ?>"
									  name="<?php echo $widget->get_field_name('type-' . $post_key); ?>"/>
								<label
									for="<?php echo $widget->get_field_id('type-' . $post_key); ?>"><?php echo stripslashes($custom_post->labels->name) ?></label>
							</p>
							<?php
							unset($post_key);
							unset($custom_post);
					...
					<h4 class="dw_toggle" style="cursor:pointer;"><?php _e('カスタム投稿タイプアーカイブ', 'display-widgets') ?>
						+/-</h4>
					<div class="dw_collapse">
						<?php foreach ($this->cposts as $post_key => $custom_post) {
							...
							?>
							<p><input class="checkbox"
									  type="checkbox" <?php checked($instance['type-' . $post_key . '-archive'], true) ?>
									  id="<?php echo $widget->get_field_id('type-' . $post_key . '-archive'); ?>"
									  name="<?php echo $widget->get_field_name('type-' . $post_key . '-archive'); ?>"/>
								<label
									for="<?php echo $widget->get_field_id('type-' . $post_key . '-archive'); ?>"><?php echo stripslashes($custom_post->labels->name) ?><?php _e('アーカイブ', 'display-widgets') ?></label>
							</p>
						<?php } ?>
					</div>
				<?php } ?>
				...
				<div class="dw_collapse">
					<?php foreach ($this->cats as $cat) {
						...
						?>
						<p><input class="checkbox"
								  type="checkbox" <?php checked($instance['cat-' . $cat->cat_ID], true) ?>
								  id="<?php echo $widget->get_field_id('cat-' . $cat->cat_ID); ?>"
								  name="<?php echo $widget->get_field_name('cat-' . $cat->cat_ID); ?>"/>
							<label
								for="<?php echo $widget->get_field_id('cat-' . $cat->cat_ID); ?>"><?php echo $cat->cat_name ?></label>
						</p>
						<?php
						unset($cat);
					}
				...
				<?php if (!empty($this->taxes)) { ?>
					...
					<div class="dw_collapse">
						<?php foreach ($this->taxes as $tax) {
							...
							?>
							<p><input class="checkbox" type="checkbox" <?php checked($instance['tax-' . $tax], true) ?>
									  id="<?php echo $widget->get_field_id('tax-' . $tax); ?>"
									  name="<?php echo $widget->get_field_name('tax-' . $tax); ?>"/>
								<label
									for="<?php echo $widget->get_field_id('tax-' . $tax); ?>"><?php echo str_replace(array('_', '-'), ' ', ucfirst($tax)) ?></label>
							</p>
							<?php
							unset($tax);
						}
				...
				<?php if (!empty($this->langs)) { ?>
					...
					<div class="dw_collapse">
						<?php foreach ($this->langs as $lang) {
							...
							?>
							<p><input class="checkbox" type="checkbox" <?php checked($instance['lang-' . $key], true) ?>
									  id="<?php echo $widget->get_field_id('lang-' . $key); ?>"
									  name="<?php echo $widget->get_field_name('lang-' . $key); ?>"/>
								<label
									for="<?php echo $widget->get_field_id('lang-' . $key); ?>"><?php echo $lang['native_name'] ?></label>
							</p>
				...
				<p><label
						for="<?php echo $widget->get_field_id('other_ids'); ?>"><?php _e('上記に記載されていない投稿のIDのリストを区切りで入力してください。：', 'display-widgets') ?>
						:</label>
					<input type="text" value="<?php echo $instance['other_ids'] ?>"
						   name="<?php echo $widget->get_field_name('other_ids'); ?>"
						   id="<?php echo $widget->get_field_id('other_ids'); ?>"/>
				</p>
			</div>
			<hr>
			...
			<p><label
					for="<?php echo $widget->get_field_id('mobile_detect_disp'); ?>"><?php _e('スマートフォンからのアクセスの場合表示しない', 'display-widgets') ?>
					:</label>
				<input type="checkbox" value="true" name="<?php echo $widget->get_field_name('mobile_detect_disp'); ?>"
					   id="<?php echo $widget->get_field_id('mobile_detect_disp'); ?>" <?php checked('true', $instance['mobile_detect_disp'], true) ?>/>
			</p>
		</div>
		<?php
	}
