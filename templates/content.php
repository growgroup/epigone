...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'panel' ); ?>>
	...
	<?php if ( is_search() || is_archive() || is_home() || is_front_page() ) : // Only display Excerpts for Search ?>
		<div class="entry-summary">
			<?php
			the_post_thumbnail(); ?><?php
			the_excerpt(); ?>
		</div><!-- .entry-summary -->    <?php else : ?>
		<div class="entry-content">
			<?php
			the_post_thumbnail(); ?><?php
			the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'epigone' ) ); ?><?php
	...
	<footer class="entry-footer">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?><?php
			...
			</span>            <?php endif; // End if categories ?>
			...
			if ( $tags_list ) :
				...
				<?php printf( __( 'Tagged %1$s', 'epigone' ), $tags_list ); ?>
			</span>            <?php endif; // End if $tags_list ?><?php endif; // End if 'post' == get_post_type() ?>
